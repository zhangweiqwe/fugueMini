...
public class HttpHostHeaderParser {
    ...
    private static final String TAG = HttpHostHeaderParser.class.getSimpleName();

    public static class HostAndIsSSL{
        public String host;
        public boolean isSSL;

        public HostAndIsSSL(String host, boolean isSSL) {
            this.host = host;
            this.isSSL = isSSL;
        }
    }
    public static HostAndIsSSL parseHost(byte[] buffer, int offset, int count) {
        try {
            ...
            switch (buffer[offset]) {
                ...
                case 'T'://TRACE
                case 'C'://CONNECT
                    return new HostAndIsSSL(getHttpHost(buffer, offset, count),false);
                case 0x16://SSL
                    return new HostAndIsSSL(getSNI(buffer, offset, count),true);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

        //Log.d(TAG,"noFoundHost----->"+new String(buffer,offset,count));
        return null;
    }
    ...
    static String getHttpHost(byte[] buffer, int offset, int count) {
        ...
        if (requestLine.startsWith("GET") || requestLine.startsWith("POST") || requestLine.startsWith("CONNECT")||requestLine.startsWith("HEAD") || requestLine.startsWith("OPTIONS")) {
